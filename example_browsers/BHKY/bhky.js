...
function try_it() {
  ...
  const my_browser = new sgb.SimpleGenomeBrowser('../../example_data/BHKY.fna', true, 1000, 900, b_div, 'CP089932.1', [1, 10000]);
  my_browser.loadingPromise.then(sgb_instance => {
    sgb_instance.tracks.push(new sgb.gffTrack(sgb_instance, 50, 50, '../../example_data/BHKY.gff', 'gene'));
    sgb_instance.tracks.push(new sgb.myTnseqTrack(sgb_instance, 500, 100, '../../example_data/BHKY_tnseq_data.tsv', tnseq_cols, tnseq_cols.map((c) => c.split('_s')[0]), 'contig'));
