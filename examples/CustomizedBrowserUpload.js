...
import { geneTrack } from "../src/SGB_tracks.js";
import { gffTrack } from "../src/data_tracks.js";
...
class phastestRegionTrack extends geneTrack {
  ...
  constructor(browser, name, h, top, config, phastest_file) {
    super(browser, name, h, top, config, 'scaffoldId', 'begin', 'end', 'locusId')
    const self = this;
...
async function load_browser(strain, fasta_file, gff_file, phastest_file) {
  ...
  my_browser.loadingPromise.then(sgb_instance => {
    sgb_instance.add_track(new customColorsGFFtrack(sgb_instance, 'GFF track', 50, 150, {}, gff_file));
    sgb_instance.add_track(new phastestRegionTrack(sgb_instance, 'PHASTEST annotations', 50, 200, {'load_threshold':100000000}, phastest_file));
  })
}
