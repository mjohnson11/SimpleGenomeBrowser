...
class quantitativeYaxesTrack extends baseTrack {
  ...
  async load_region() {
    ...
    for (let i=0; i<self.columnNames.length; i++) {
      ...
      let tmp_domain = d3.extent(data.map((r) => r[self.columnNames[i]]));
      let tmp_config = {
        domain: tmp_domain,
        yticks: [tmp_domain[0], (tmp_domain[1]+tmp_domain[0])/2, tmp_domain[1]],
        ytick_formatter: (d => d==0.1 ? 0 : d),
        clip: false,
        log_y: false
      };
      if ((self.config.column_config) && (self.config.column_config[self.columnNames[i]])) {
        // overwrite settings from config
        tmp_config = {...tmp_config, ...self.config.column_config[self.columnNames[i]]};
      }

      let bottom = (yrange_size+fraction_buf)*(i+1);
      let top = bottom-yrange_size;
      let tmp_scale = (tmp_config.log_y) ? d3.scaleLog() : d3.scaleLinear();
      tmp_scale = (tmp_config.clip) ? tmp_scale.range([bottom, top]).clamp(true).domain(tmp_config.domain).nice() : tmp_scale.range([bottom, top]).domain(tmp_config.domain).nice();
      yScales.push(tmp_scale);

      self.axis_elements.selectAll('.yline'+String(i))
        .data(tmp_config.yticks)
        .enter()
        .append('line')
          ...
          .attr('stroke', 'black')
          .attr('stroke-width', d => (d == tmp_config.yticks[0]) ? 1 : 0.25)
          
      column_configs.push(tmp_config);
    }
    ...
    self.axis_elements.append('rect')
      ...
      .attr('height', self.h)
      .attr('fill', 'white')
    // two loops so this gets on top of the background
    for (let i=0; i<self.columnNames.length; i++) {
      let tmp_config = column_configs[i]
      const yAxis = d3.axisLeft(yScales[i])
        .tickValues(tmp_config.yticks)
        .tickFormat(tmp_config.ytick_formatter)
        .tickSize(0)
      ...
      self.axis_elements.append('text')
        ...
        .attr("y", yScales[i](tmp_config.yticks[tmp_config.yticks.length-1])-5)
